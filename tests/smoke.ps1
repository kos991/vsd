...
Assert-FileDoesNotContain 'overlay/usr/local/sbin/daed-firstboot' 'strong_password|prompt_strong_secret' 'firstboot must not enforce password complexity beyond non-empty matching values.'
Assert-FileDoesNotContain 'overlay/usr/local/sbin/daed-firstboot' 'daed admin username|daed admin password|firstboot-admin\.env' 'firstboot must not ask for or save daed admin credentials.'
Assert-FileDoesNotContain 'overlay/usr/local/sbin/daed-firstboot' 'createUser|/graphql|start_firstboot_daed|--api-only|numberUsers' 'firstboot must not pre-create daed users through the GraphQL API.'
Assert-FileContains 'overlay/etc/init.d/daed' '--env HOME=/root --env XDG_DATA_HOME=/root/\.local/share' 'normal daed service must keep a stable data home for dashboard-created users.'
Assert-FileContains 'overlay/usr/local/sbin/daed-firstboot' 'Create the daed admin user in the dashboard' 'firstboot must tell users to create the daed admin account in the official dashboard.'
