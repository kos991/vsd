...
Assert-FileContains 'scripts/build-debian-ova.sh' 'deb \[trusted=yes\] http://deb\.xanmod\.org \$\{DEBIAN_CODENAME\} main' 'Debian build must add the XanMod apt repository for the selected Debian codename.'
...
Assert-FileContains 'scripts/build-debian-ova.sh' 'systemctl enable gateway-network-init\.service' 'Debian build must enable gateway network initialization.'
Assert-FileContains 'overlay-debian/etc/systemd/system/gateway-network-init.service' 'Before=check-ebpf\.service daed\.service' 'Gateway network init must run before eBPF checks and daed.'
